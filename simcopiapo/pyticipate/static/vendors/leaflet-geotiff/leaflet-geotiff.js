...
L.LeafletGeotiff = L.ImageOverlay.extend({
    ...
    setData: function(arrayBuffer) {
        this._parseTIFF(arrayBuffer);
    },
    ...
    getMinMax: function() {
        ...
        var rows = this.getRasterRows();
        var cols = this.getRasterCols();
        var data = this.getRasterArray();
        for (var r = 0; r < rows; r++) {
            for (var c = 0; c < cols; c++) {
                var v = data[r*cols+c];
                if (v !== this.options.noData) {
                    if (v < min) {
                        min = v;
                    }
                    if (v > max) {
    ...
    onAdd: function (map) {
        ...
        this._getPane(map).appendChild(this._image);
    ...
    onRemove: function (map) {
        this._getPane(map).removeChild(this._image);
    ...
    _onMouseMove: function(e) {
        var v = this.getValueAtLatLng(e.latlng.lat, e.latlng.lng);
        if (v === undefined || this._isNull(v) || this._isHidden(v) ) {
    ...
    _onClick: function(e) {
        var v = this.getValueAtLatLng(e.latlng.lat, e.latlng.lng);
        if (v === undefined || this._isNull(v) || this._isHidden(v) ) { return; }
        e.value = v;
        this.fire("click", e);
    },
    _isNull: function(v) {
        if (this.options.renderer) {
            return v === this.options.renderer.options.noData;
        }
        return false;
    },
    _isHidden: function(v) {
        if (this.options.renderer) {
            var tooLow = false;
            if (!this.options.renderer.options.clampLow) {
                tooLow = v < this.options.renderer.options.displayMin;
            }
            var tooHigh = false;
            if (!this.options.renderer.options.clampHigh) {
                tooHigh = v > this.options.renderer.options.displayMax;
            }
            return (tooLow || tooHigh);
        }
        return false;
    },
    _getPane: function(map) {
        if (this.options.pane != null && this.options.pane != '') {
            return map.getPane(this.options.pane);
        }
        else {
            return map._panes.overlayPane;
        }
    },
    getRowCol: function(lat, lng) {
        var col = Math.floor(this.raster.width*(lng - this._rasterBounds._southWest.lng)/(this._rasterBounds._northEast.lng - this._rasterBounds._southWest.lng));
        var row = this.raster.height-Math.ceil(this.raster.height*(lat - this._rasterBounds._southWest.lat)/(this._rasterBounds._northEast.lat - this._rasterBounds._southWest.lat));
        return [row,col];
    },
    _getData: function() {
        var self = this;
